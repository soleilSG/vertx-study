...
public class MainVerticle extends AbstractVerticle {
	...
	private Future<Void> prepareDatabase() {
		...
		dbClient = JDBCClient.createShared(vertx, new JsonObject()
				.put("url", "jdbc:mysql://47.104.175.9:4306/vertx_test?useUnicode=true&characterEncoding=utf-8&useSSL=false")
				.put("driver_class", "com.mysql.jdbc.Driver")
				.put("user", "tokamak")
				.put("password", "tokamak123-DB")
		...
		dbClient.getConnection(ar -> {
			if (ar.failed()) {
				...
				LOGGER.debug("MySQL DB Connection is OK!");
				future.complete();
				/*
				...
				connection.execute(SQL_CREATE_PAGES_TABLE, create -> {
					connection.close();
					if (create.failed()) {
						LOGGER.error("Database prepartion error", create.cause());
	...
	private Future<Void> startHttpServer() {
		...
		server.requestHandler(router::accept).listen(8090, ar -> {
			if (ar.succeeded()) {
